...
import me.roundaround.trove.observable.Computed;
...
public class State implements AutoCloseable {
  ...
  public State(@NotNull PackData pack, @NotNull PackData lastSavedPack) {
    ...
    this.subscriptions.add(this.images.pairwise().subscribe((t) -> {
  ...
  private static class TextureManager {
    ...
    private static final HashMap<Identifier, DynamicTexture> textures = new HashMap<>();
    ...
    public static Identifier registerTexture(Image image) {
      ...
      try {
        ...
        if (refCount == 1) {
          DynamicTexture texture = new DynamicTexture(() -> image.hash(), image.toNativeImage());
